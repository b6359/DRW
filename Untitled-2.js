...
function initializeProductMaster() {
  ...
  function initializeItemRow(row) {
    const input = row.querySelector(".itemDatalistInput");
  ...
  document.addEventListener("click", e => {
    ...
    if (e.target.classList.contains("addMoreBtn")) {
      ...
      if (itemRows.length > 0) {
        ...
        const input = lastRow.querySelector(".itemDatalistInput");
        const qtyInput = lastRow.querySelector(".itemQtyInput");

        if (!input.value || !qtyInput.value || parseInt(qtyInput.value) <= 0) {
...
==============================
   ...
   <div style="display: flex; align-items: center; position: relative; width: 100%; gap: 10px;">
            <!-- Datalist-based input -->
            <input list="itemList" class="itemDatalistInput" placeholder="Search item..."
                style="flex: 1; min-width: 300px; font-size: 14px; padding: 8px; border: 1px solid grey; border-radius: 20px; outline: none;" required />
